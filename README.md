<h1 align="center">Hi, I'm Shivani Tiwari 👋</h1>
<h3 align="center">Business Development Executive → Business Analyst | Web2 & Web3</h3>

<p align="center">
  <a href="shivanitiwarist12@gmail.com"><img src="https://img.shields.io/badge/Email-EA4335?style=for-the-badge&logo=gmail&logoColor=white" alt="Email"/></a>
  <a href="https://www.linkedin.com/in/shivani-tiwari-66ba141a6/"><img src="https://img.shields.io/badge/LinkedIn-0A66C2?style=for-the-badge&logo=linkedin&logoColor=white" alt="LinkedIn"/></a>
  <a href="https://github.com/r2athena"><img src="https://img.shields.io/badge/GitHub-181717?style=for-the-badge&logo=github&logoColor=white" alt="GitHub"/></a>
</p>

🧭 About Me

- 🚀 Business Development Executive with 2+ years in B2B client engagement, now transitioning into a Business Analyst role.
- 🌐 Worked across Web2 and Web3 services - market research, outreach & pipeline tracking, sales-trend analysis, and client proposals.
- 🧩 Comfortable bridging technical and business teams, backed by a B.Tech in Computer Science.
- 📊 Currently leveling up in SQL, Power BI, and core BA process work - BRD/FRD writing and requirements gathering.
- 🤝 Mentored junior executives and collaborate closely with product, engineering, and marketing teams.

🛠️ Skills & Tools

Data & Analytics:

![Excel](https://img.shields.io/badge/Microsoft_Excel-217346?style=flat&logo=microsoft-excel&logoColor=white)
![SQL](https://img.shields.io/badge/SQL-4479A1?style=flat&logo=postgresql&logoColor=white)
![Power BI](https://img.shields.io/badge/Power_BI-F2C811?style=flat&logo=powerbi&logoColor=black)
![Google Sheets](https://img.shields.io/badge/Google_Sheets-34A853?style=flat&logo=googlesheets&logoColor=white)

Business Analysis:

![Requirements](https://img.shields.io/badge/Requirements_Gathering-6E56CF?style=flat)
![BRD/FRD](https://img.shields.io/badge/BRD_&_FRD_Writing-6E56CF?style=flat)
![Market Research](https://img.shields.io/badge/Market_Research-6E56CF?style=flat)
![Gap Analysis](https://img.shields.io/badge/Gap_&_Root_Cause_Analysis-6E56CF?style=flat)

Programming & AI:

![Python](https://img.shields.io/badge/Python-3776AB?style=flat&logo=python&logoColor=white)
![PostgreSQL](https://img.shields.io/badge/PostgreSQL-4169E1?style=flat&logo=postgresql&logoColor=white)
![MySQL](https://img.shields.io/badge/MySQL-4479A1?style=flat&logo=mysql&logoColor=white)
![Git](https://img.shields.io/badge/Git-F05032?style=flat&logo=git&logoColor=white)
![GitHub](https://img.shields.io/badge/GitHub-181717?style=flat&logo=github&logoColor=white)

Tools & Methodologies:

![Postman](https://img.shields.io/badge/Postman-FF6C37?style=flat&logo=postman&logoColor=white)
![Trello](https://img.shields.io/badge/Trello-0052CC?style=flat&logo=trello&logoColor=white)
![Agile](https://img.shields.io/badge/Agile-0078D4?style=flat)
![Scrum](https://img.shields.io/badge/Scrum-009FDA?style=flat)

Domain: Web2 · Web3 (Blockchain, Smart Contracts, DeFi) · DevOps · B2B SaaS · Developer Tooling

💼 Experience

Senior Business Development Executive - Syvora (Mar 2025 - Present)
- Ran competitive analysis and market research across the Web2, Web3 and crypto ecosystem; turned findings into briefs and decision memos for product and leadership.
- Ran stakeholder interviews and discovery sessions; documented business and functional requirements to feed roadmap conversations.
- Built and maintained pipeline reports in Excel (pivot tables, trend analysis) and used weekly KPI reviews to spot drop-offs and refine the outreach playbook.
- Mentored junior executives on data hygiene, Excel reporting standards, and requirements-writing basics.

Business Development Executive - Syvora (Dec 2023 - Feb 2025)
- Generated leads across LinkedIn, Twitter, Telegram and crypto fundraising channels for development projects.
- Maintained client data and analysed weekly/quarterly sales trends in Excel.
- Wrote business proposals, project costings, and pitch decks; ran client meetings and negotiations.

Analyst (A4), Intern - Capgemini (Jan 2023 – Jun 2023)
- Gathered and documented requirements for client applications; turned stakeholder calls into structured requirement documents.
- Built pivot tables and summary views for weekly status reviews; prepared process flows and user stories in an Agile setup.
- Supported UAT - logged defects, tracked fixes, and kept documentation current.

🚧 Projects

🏨 Hotel Management System (Feb 2023 - Jun 2023)
Full-stack hotel booking app - Java, Spring Boot, Spring JPA, PostgreSQL backend with HTML/CSS/Bootstrap front end. Designed the data model for rooms, users, and bookings; added Spring Security for auth.

🎵 Mouzikka - Music Player (May 2022 - Jun 2022)
Python desktop music player built with Tkinter and MySQL - playback controls plus a favourites feature backed by a structured database.

🎓 Certifications

📜 Postman API Fundamentals Student Expert - Postman

📈 GitHub Stats

<p align="center">
  <img height="180" src="https://github-readme-stats.vercel.app/api?username=r2athena&show_icons=true&theme=default&hide_border=true" alt="GitHub Stats"/>
  <img height="180" src="https://github-readme-stats.vercel.app/api/top-langs/?username=r2athena&layout=compact&theme=default&hide_border=true" alt="Top Languages"/>
</p>

<p align="center">
  <img src="https://github-readme-streak-stats.herokuapp.com/?user=r2athena&theme=default&hide_border=true" alt="GitHub Streak"/>
</p>

<p align="center"><i>Bridging business and engineering - one clear requirement at a time.</i></p>
